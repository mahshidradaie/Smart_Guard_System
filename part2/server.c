...
// --- UDP RECEIVER THREAD (SECURE AES-GCM + NETWORK MONITORING) ---
void *udp_video_receiver(void *arg) {
    ...
    while(1) {
        ...
        if (recv_len < 0) {
            ...
            if (!network_is_down) {
                printf("\n[NETWORK ALERT] No video stream received for 3 seconds! Waiting for recovery...\n");
                network_is_down = 1;
            }
            continue; 
        } 
        ...
        if (recv_len > 0 && network_is_down) {
            printf("[NETWORK RECOVERY] Video stream re-established successfully!\n");
            network_is_down = 0;
        }
    ...
    while(1) {
        ...
        if (recv_len > 28) {
            ...
            if (ret > 0) {
                ...
                int new_count = atoi(plaintext); 
                if (new_count != current_person_count) {
                    add_to_history(new_count);
                    current_person_count = new_count;
                }
            }
        }
    }
...
void *handle_video_stream_thread(void *arg) {
    ...
    while (1) {
        ...
        if (local_size > 0) {
            ...
            if (SSL_write(ssl, local_frame, local_size) <= 0) {
        fprintf(stderr, "[NETWORK FAULT] Connection to /API/V1/STREAM lost! SSL_write failed.\n");
        break;
    }
            if (SSL_write(ssl, "\r\n", 2) <= 0) {
                break;
...
int main() {
    ...
    while(1) {
        ...
        if (strncmp(buffer, "GET /API/V1/STREAM", 18) == 0) {
            ...
            char json[256];
            sprintf(json, "{\"cpu_usage\": %.1f, \"free_mem\": %ld, \"cpu_temp\": %.1f}", 
                    get_cpu_usage(), get_free_memory(), get_cpu_temp());
        ...
        // 5. REST API: POST /API/V1/COMMAND 
        else if (strncmp(buffer, "POST /API/V1/COMMAND", 20) == 0) {
            // Send success response immediately so the client doesn't hang
            ...
            // Execute the reboot if the JSON payload contains the command
            if (strstr(buffer, "reboot") != NULL) {
                printf("\n[API] Reboot command received! Restarting system...\n");
                system("/usr/bin/sudo /sbin/reboot > /tmp/reboot_error.log 2>&1");
            }
        }
        else {
            int fd = SSL_get_fd(ssl); SSL_shutdown(ssl); SSL_free(ssl); close(fd);
